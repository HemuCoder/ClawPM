...
1. 命中项目或长期主题时，先读取 `memory/topics/index.md`。
2. 命中已有 topic 后，进入对应项目目录并优先读取 `memory/topics/<topic>/00-overview.md`。
...
9. 会话启动时如果存在 `memory/topics/index.md`，应先轻量读取一次，把它作为项目导航索引，而不是深读全部项目内容。
10. 初始化完成后的首次引导里，要提醒用户：手动 `/new` 开新会话前，先让 agent 更新本轮项目记忆。
...
- 会话启动时如果存在 `memory/topics/index.md`，先轻量读取一次，把它作为项目导航索引。
- 命中项目或长期主题时，先读取 `memory/topics/index.md`。
- 命中已有 topic 后，进入对应项目目录并优先读取 `memory/topics/<topic>/00-overview.md`。
